...
namespace WinTcare
{
    public partial class memPayroll : Form
    {
        ...
        {
            string cglquery = "udpate glmast set lintdate = @dTransactionDate where cacctnumb = @acctNumb";
                        ...
                        using (SqlConnection ndConnHandle3 = new SqlConnection(cs))
                        {
                            ...
                            foreach (DataGridViewRow dr in acctGrid.Rows)
                            {
                                ...
                                if(lready2go)                               
                                {
                                    ...
                                    if (tcAcctNumb != "")
                                    {
                                        int tnProductID = getProductId(tcAcctNumb, cs);
  ...
  //                          MessageBox.Show("We are going for the next account for " + tcpayroll+" and the count is "+i);
    ...
    //                    MessageBox.Show("We are done with member with payroll " + tcpayroll);
        ...
        private void postMemberAccounts(string tcAcctNumb, decimal lnTranAmt,decimal lnAccrInt, string tccontra, int prid)
        {
            ...
            string tcChqno = "";
            string tcTranCode = (dacode == "250" || dacode == "251" || dacode == "270" || dacode == "271" ? "01" : "07");
            bool llPaid = false;
            int tnqty = 1;
            string tcReceipt = "";
            bool llCashpay = true;
            ...
            if (tcTranCode == "07")
            {
                ...
                tls1.updCuTranhist(cs, tcAcctNumb, tnPrinAmt, tcDesc, tcVoucher, tcChqno, tcUserid, tnPNewBal, tcTranCode, lnServID, llPaid, tccontra, lnWaiveAmt, tnqty, unitprice, tcReceipt, llCashpay, visno, isproduct,
                ...
                tls1.updCuTranhist(cs, tccontra, tnPrinCont, tcDesc, tcVoucher, tcChqno, tcUserid, tnCNewBal, tcTranCode, lnServID, llPaid, tcAcctNumb, lnWaiveAmt, tnqty, unitprice, tcReceipt, llCashpay, visno, isproduct,
                ...
                upj.updJournal(cs,tccontra, tcDesc, tnPrinCont, tcVoucher, tcTranCode, gdTransactionDate,globalvar.gcUserid,globalvar.gnCompid);
                upj.updJournal(cs,gcControlAcct, tcDesc, tnPrinAmt, tcVoucher, tcTranCode, gdTransactionDate, globalvar.gcUserid, globalvar.gnCompid);
                ...
                decimal tnAccruedPost = -Math.Abs(lnAccrInt); 
                ...
                tcCustcode = tcAcctNumb.Substring(3, 6);
                tls1.updCuTranhist(cs, tcAcctNumb, tnAccruedPost, tcDesc, tcVoucher, tcChqno, tcUserid, tnPNewBal0, tcTranCode, lnServID, llPaid, tccontra, lnWaiveAmt, tnqty, unitprice, tcReceipt, llCashpay, visno, isproduct,
                srvid, "", "", lFreeBee, tcCustcode, ncompid, globalvar.gnBranchid, globalvar.gnCurrCode, gdTransactionDate, gdTransactionDate);                          //update tranhist posting account
                string auditDesc1 = "Accrued Interest Charged " + tcAcctNumb; //Audit Trail
                ...
                decimal tnAccruedCont = Math.Abs(lnAccrInt); 
                gls.updGlmast(cs, tcAcctNumb, tnAccruedCont);                              ////update accrued interest paid or interest paid - Credit
                decimal tnPNewBal1 = CheckLastBalance.lastbalance(cs, tcAcctNumb);     
                tcCustcode = tcAcctNumb.Substring(3, 6);
                tls1.updCuTranhist(cs, tcAcctNumb, tnAccruedCont, tcDesc, tcVoucher, tcChqno, tcUserid, tnPNewBal1, tcTranCode, lnServID, llPaid, tccontra, lnWaiveAmt, tnqty, unitprice, tcReceipt, llCashpay, visno, isproduct,
                srvid, "", "", lFreeBee, tcCustcode, ncompid, globalvar.gnBranchid, globalvar.gnCurrCode, gdTransactionDate, gdTransactionDate);                          //update tranhist posting account
                auditDesc1 = "Accrued Interest Paid " + tcAcctNumb;   //Audit Trail
                ...
                tcCustcode = tccontra.Substring(3, 6);
                tls1.updCuTranhist(cs, tccontra, tnAccruedPost, tcDesc, tcVoucher, tcChqno, tcUserid, tnCNewBal2, tcTranCode, lnServID, llPaid, tcAcctNumb, lnWaiveAmt, tnqty, unitprice, tcReceipt, llCashpay, visno, isproduct,
                srvid, "", "", lFreeBee, tcCustcode, ncompid, globalvar.gnBranchid, globalvar.gnCurrCode, gdTransactionDate, gdTransactionDate);                        //update tranhist account 
            ...
            else
            {
                ...
                int tnProductID = getProductId(tcAcctNumb, cs);
                gcControlAcct = getProductControl.productControl(cs, tnProductID);
//                gcControlAcct = (tcAcctNumb.Substring(0, 3) == "250" || tcAcctNumb.Substring(0, 3) == "251") ? globalvar.gcSaveCtrl_Ind : globalvar.gcShareCtrl_Ind; 
                ...
                upj.updJournal(cs,tccontra, tcDesc, tnContAmt, tcVoucher, tcTranCode, gdTransactionDate, globalvar.gcUserid, globalvar.gnCompid);
                upj.updJournal(cs,gcControlAcct, tcDesc, tnContAmt, tcVoucher, tcTranCode, gdTransactionDate, globalvar.gcUserid, globalvar.gnCompid);
                updateClient_Code updcl = new updateClient_Code();
                updcl.updClient(cs, "nvoucherno");
                updcl.updClient(cs, "stackcount");
            }
