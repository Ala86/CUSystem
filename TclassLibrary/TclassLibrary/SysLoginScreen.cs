...
{
    public partial class SysLoginScreen : Form
    {
        string cs = string.Empty;
        int ncompid = 0;
        string dloca = string.Empty;
        bool glCompanyOk = false;
        bool glGroupOk = false;
        bool glPersonalOk = false;
        bool glSuspensOk = false;
        ...
        //bool glPersonalOk = false;
        //bool glSuspensOk = false;
        public SysLoginScreen(string tcCos, int tnCompid, string tcLoca,string bkgrImage,string diconfile)
        {
            InitializeComponent();
            cs = tcCos;
            ncompid = tnCompid;
        ...
        private void SysLoginScreen_Load(object sender, EventArgs e)
        {
            ...
            using (SqlConnection nConnHandle = new SqlConnection(cs))
            {
                ...
                if (cUserDetails.HasRows == true)
                {
                    ...
                    if (duser == finUserID && dpass == finUserPASS)
                    {
                        ...
                        if (glCompanyOk && glGroupOk && glPersonalOk && glSuspensOk)
                        {
                            //this.Hide();
                        ...
                        {
                            MessageBox.Show("Incomplete User Credentials");
                            this.UserID.Text = "";
...
//            }
        ...
        private void getGroupbelong(int nusernumb)
        {
            ...
            using (SqlConnection ndConnHandle = new SqlConnection(cs))
            {
                ...
                else { glGroupOk = false; MessageBox.Show("user does not belong to any group with access rights, inform IT DEPT immediately"); }
            }
        }

        private void inheritedRights(int tngrpid, int tnroleid, int tnUserNumb)
        {
            string dsqlinhRites = "exec Tsp_getUserInheritedRights " + ncompid + "," + tngrpid + "," + tnroleid + "," + tnUserNumb; 
            using (SqlConnection ndConnHandle = new SqlConnection(cs))
            {
                ndConnHandle.Open();
                ...
                }//else { MessageBox.Show("No inherited rights for this user for "); }
            }
        }
    ...
    select nic.GetPhysicalAddress().ToString()
        ...
        private void SuspenseAccount()
        {
            using (SqlConnection nConnHandle = new SqlConnection(cs))
            {
                ...
                }else { glSuspensOk = false; MessageBox.Show("Internal Suspense Account not found, inform IT DEPT immediately"); }
            }
        }
...
//                    MessageBox.Show("The company has been found step 3");
                ...
                }else { glCompanyOk = false; MessageBox .Show("Company details incomplete, inform IT DEPT immediately"); }
            }
        }
...
//                        MessageBox.Show("The transfer flag in loginscreen is " + globalvar.glTransfers);
                ...
                else { glPersonalOk = false; MessageBox.Show("no personal rights defined, inform IT DEPT immediately"); }
            }
        }
