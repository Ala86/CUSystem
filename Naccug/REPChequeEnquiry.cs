...
namespace WinTcare
{
    public partial class REPChequeEnquiry : Form
    {
        string cs = globalvar.cos;

        ReportDocument rprt = new ReportDocument();
        public REPChequeEnquiry()
        {
            InitializeComponent();
        }

        private void REPChequeEnquiry_Load(object sender, EventArgs e)
        {
            BranchCombo();
            CurrencyCombo();
        }
        ...
        public void GETDATA()
        {
            ...
            SqlConnection ndConnHandle1 = new SqlConnection(cs);
            ndConnHandle1.Open();
            rprt.Load(System.Windows.Forms.Application.StartupPath + "\\Reports\\CRPTransactionCheque.rpt");
            SqlCommand cmd = new SqlCommand("SPChequeTran", ndConnHandle1);
            cmd.CommandType = CommandType.StoredProcedure;
        ...
        {
            string che = "ChequeOut";
            if (checkBox2.Checked)
            {
                textBox8.Text = che.Trim();
            }
            else
            {
                textBox8.Text = "";
