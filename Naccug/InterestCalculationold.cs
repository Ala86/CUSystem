              ...
              //  MessageBox.Show("");
                ...
                if (minbalview != null && minbalview.Rows.Count > 0)
                {
                    ...
                    for (int i = 0; i < minbalview.Rows.Count; i++)
                    {
                        ...
                        {
                            tnMinBal = tnMinBal + calcminBal(dacct, startMonth+z, startyear);
...
//                        MessageBox.Show("Minimum balance for " + dacct + " = " + tnMinBal);
        ...
        {
            string dsql = "update glmast set nminbal=0.00 where acode in ('250','251','270','271') ";
            using (SqlConnection nConnHandle2 = new SqlConnection(cs))
            {
        ...
        private decimal calcminBal(string tcAct,int dmonth, int dyear)
        {
            using (SqlConnection nConnHandle2 = new SqlConnection(cs))
            {
              string minbalsql = " select nminbal = case when min(nnewbal) is not null then min(nnewbal) else 0.00 end from tranhist where cacctnumb = @dact AND MONTH(dpostdate) = @dmth and YEAR(dpostdate) = @dyr and Nnewbal > 0.00";
...
//                    MessageBox.Show("The minimum balance for month " + dmonth + ", account " + tcAct + " = " + dminbal);
            ...
            using (SqlConnection nConnHandle2 = new SqlConnection(cs))
            {
                   ...
                   
                {
                   ...
                   // MessageBox.Show("this is" + clrow);
                    if (tlSelected)
                    {
                        string dsql20 = "select cacctnumb, prd_id from glmast where intcode=0 and acode in ('250','270')";
        ...
        private void getIntrate(int lnpridid)
        {
            using (SqlConnection nConnHandle2 = new SqlConnection(cs))
            {
               ...
               //     gnSaveProduct = lnpridid;
                  //   MessageBox.Show("this is" + gnSaveProduct);
                  ...
                  //  MessageBox.Show("You are " + gcSavingsExpense);
                    if (gcSavingsExpense != "")
                    {
                        ...
                        if (intcalview != null && intcalview.Rows.Count > 0)
                        {
                            ...
                            upj.updJournal(cs, gcControlAcct, tcDesc, Math.Abs(tnTotalAccrued), tcVoucher, tcTranCode, globalvar.gdSysDate, globalvar.gcUserid, globalvar.gnCompid);
        ...
        private void postAccounts(string tcAcctNumb, decimal lnTranAmt, string ccontra)
        {
          ...
          //  AuditTrail au = new AuditTrail();
            ...
            upj.updJournal(cs, tcContra, tcDesc, -Math.Abs(tnTranAmt), tcVoucher, tcTranCode, dateTimePicker3.Value, globalvar.gcUserid, globalvar.gnCompid);
            upj.updJournal(cs, tcAcctNumb, tcDesc, Math.Abs(tnTranAmt), tcVoucher, tcTranCode, dateTimePicker3.Value, globalvar.gcUserid, globalvar.gnCompid);
