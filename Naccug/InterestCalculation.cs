...
namespace WinTcare
{
    public partial class InterestCalculation : Form
    {
        ...
        decimal gnLastYearBalance = 0.00m;
        ...
        private void button1_Click(object sender, EventArgs e)
        {
...

            {
                ...
                if (tlSelected)
                {
                    ...
                    {
                        int startYear = dateFrom.Value.Year;
                        int lastYear = startYear - 1;
                        int startMonth = dateFrom.Value.Month;
                        int endMonth = dateTo.Value.Month;
                        int numMonths = endMonth - startMonth;
                        decimal tnMinBal = 0.00m;
                        decimal currMinBal = 0.00m;
                         //  MessageBox.Show("this is the orginal Start " + startMonth);
                       //    MessageBox.Show("this is the orginal End "+ endMonth);
                     //   MessageBox.Show("this is the orginal NumberOfMonth " + numMonths);
                        string dsql2 = "select cacctnumb from glmast  where intcode = 0 and acode in ('250','251','270','271') and caccstat <> 'C'";
                        ...
                        if (minbalview != null && minbalview.Rows.Count > 0)
                        {
                            ...
                            for (int i = 0; i < minbalview.Rows.Count; i++)
                            {
                                ...
                                getLastYearBalance(dacct, lastYear);
                                tnMinBal = gnLastYearBalance;
                                currMinBal = gnLastYearBalance;
                                //   MessageBox.Show("this is the orginal Cacctnumb " + dacct);
                                //   MessageBox.Show("this is the orginal currMinBal "+currMinBal);
                                // for (int z = 1; z <= 12; z++)
                                for (int z = startMonth + 1; z <= endMonth; z++)
                                {
                                    decimal lnTempMinBal = minBalCalc(dacct, startYear, z);
                                  //   MessageBox.Show("value for month " + z+" is "+lnTempMinBal);
                                    currMinBal = lnTempMinBal == 0.00m ? currMinBal :                          //the returned balance is 0, meaning not value for current month
                                   lnTempMinBal > 0.00m && lnTempMinBal > currMinBal ? lnTempMinBal : currMinBal;
                                    //    lnTempMinBal > 0.00m && lnTempMinBal <= currMinBal ? lnTempMinBal : currMinBal;
                                    tnMinBal = tnMinBal + currMinBal;
                                    // tnMinBal = currMinBal;
                                    //    MessageBox.Show("The final currMinBal = " + currMinBal);
        ...
        private void getLastYearBalance(string tcAct, int tnYr)
        {
            using (SqlConnection ndcon = new SqlConnection(cs))
            {
                gnLastYearBalance = 0.00m;
                string minbalsql = "exec tsp_getLastYearBal @compid, @dyr,@dact";
                SqlDataAdapter selCommand = new SqlDataAdapter();
                selCommand.SelectCommand = new SqlCommand(minbalsql, ndcon);

                selCommand.SelectCommand.Parameters.Add("@compid", SqlDbType.Int).Value = ncompid;
                selCommand.SelectCommand.Parameters.Add("@dyr", SqlDbType.Int).Value = tnYr;
                ...
                {
                    gnLastYearBalance = Convert.ToDecimal(balview.Rows[0]["nminbal"]);
                }
                ndcon.Close();
            }
        }

        private decimal minBalCalc(string tcAct, int dyear,int dmonth)
        {
            using (SqlConnection nConnHandle2 = new SqlConnection(cs))
            {
                string minbalsql = "exec tsp_getMinBal @compid, @dyr,@dmth,@dact";
                SqlDataAdapter selCommand = new SqlDataAdapter();
                ...
                selCommand.SelectCommand.Parameters.Add("@dyr", SqlDbType.Int).Value = dyear;
                DataTable balview = new DataTable();
                nConnHandle2.Open();
                selCommand.SelectCommand.ExecuteNonQuery();
                nConnHandle2.Close();
        ...
        {
            string dsql = "update glmast set nminbal=0.00 where acode in ('250','251','270','271') and caccstat <> 'C' ";
            using (SqlConnection nConnHandle2 = new SqlConnection(cs))
            {
        ...
        private decimal calcminBal(string tcAct,int dmonth,int dyear)
        {
            using (SqlConnection nConnHandle2 = new SqlConnection(cs))
            {
                string minbalsql = " select nminbal = case when min(nnewbal) is not null then min(nnewbal) else 0.00 end from tranhist where cacctnumb = @dact AND MONTH(dpostdate) = @dmth and year(dpostdate)=@dyr";
                SqlDataAdapter selCommand = new SqlDataAdapter();
                selCommand.SelectCommand = new SqlCommand(minbalsql, nConnHandle2);
            ...
            using (SqlConnection nConnHandle2 = new SqlConnection(cs))
            {
                   ...
                   
                {
                  ...
                  //  MessageBox.Show("this is" + clrow);
                    if (tlSelected)
                    {
                        string dsql20 = "select cacctnumb, prd_id from glmast where intcode=0 and acode in ('250','251','270','271') and caccstat <> 'C'";
                        ...
                        if (intcalview != null && intcalview.Rows.Count > 0)
                        {
                            ...
                            for (int i = 0; i < intcalview.Rows.Count; i++)
                            {
                                ...
                                int lnprid = Convert.ToInt16(intcalview.Rows[i]["prd_id"]);
                                // MessageBox.Show("You are indie"+lnprid);
                                getIntrate(lnprid);
        ...
        private void getIntrate(int lnpridid)
        {
            using (SqlConnection nConnHandle2 = new SqlConnection(cs))
            {
             
                string dsql21 = "select int_rate, sexp_acc from prodtype where prd_id="+ lnpridid;
                ...
                if (rateview != null && rateview.Rows.Count > 0)
                {
                    ...
                    gnInterestRate = Convert.ToDecimal(rateview.Rows[0]["int_rate"]);
                    gcSavingsExpense = rateview.Rows[0]["sexp_acc"].ToString();
                 //   MessageBox.Show("This is the Savings " + gcSavingsExpense);
                    textBox4.Text = gcSavingsExpense;
                    gcControlAcct = getProductControl.productControl(cs, lnpridid);
                   // MessageBox.Show("This is the Savinin product " + gcControlAcct);
                }
        ...
        private void getProduct(int lnpridid)
        {
            using (SqlConnection nConnHandle2 = new SqlConnection(cs))
            {

                string dsql21 = "select prod_Control from prodtype where prd_id=" + lnpridid;
                SqlDataAdapter da21 = new SqlDataAdapter(dsql21, nConnHandle2);
                DataTable rateview = new DataTable();
                da21.Fill(rateview);
                if (rateview != null && rateview.Rows.Count > 0)
                {
                   ...
                   // gnInterestRate = Convert.ToDecimal(rateview.Rows[0]["int_rate"]);
                    gcSavingsControl = rateview.Rows[0]["prod_Control"].ToString();
                    //   MessageBox.Show("This is the Savings " + gcSavingsExpense);
                    textBox1.Text = gcSavingsControl;
                 //   gcControlAcct = getProductControl.productControl(cs, lnpridid);
                    // MessageBox.Show("This is the Savinin product " + gcControlAcct);
                }
                else { MessageBox.Show("nothing of interest found"); }
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            using (SqlConnection nConnHandle2 = new SqlConnection(cs))
            {
                ...
                {
                    //   gcControlAcct = getProductControl.productControl(cs, gnSaveProduct);
                    //   MessageBox.Show("You are " + gcControlAcct);
                    //getIntrate(gnSaveProduct);
                    // MessageBox.Show("You are " + gcSavingsExpense);
                    //   if (gcSavingsExpense != "")
                    //   {
                    ...
                    if (tlSelected)
                    {
                        ...
                        string dsql20 = "select prd_id,glmast.cacctnumb,AccruedInterest,acode from glmast, cusreg where cusreg.ccustcode = glmast.ccustcode and AccruedInterest > 0.00 and acode in ('250','270') and caccstat <> 'C' and lactive = 1";
                        SqlDataAdapter da20 = new SqlDataAdapter(dsql20, nConnHandle2);
                        da20.Fill(intcalview);
                        if (intcalview != null && intcalview.Rows.Count > 0)
                        {
        ...
        private void postAccounts(string tcAcctNumb, decimal lnTranAmt, string ccontra)
        {
            ...
            string tcDesc = glProductDeduction ? gcProductDesc : "Interest Application";
            ...
            srvid, "", "", lFreeBee, tcCustcode, tncompid, globalvar.gnBranchid, globalvar.gnCurrCode, dProcessDate.Value, dProcessDate.Value);                          //update tranhist posting account
...
Endwith
        ...
        {
            REPMinimunBal al = new REPMinimunBal();
            al.ShowDialog();
        }

        private void button11_Click(object sender, EventArgs e)
        {
            REPInterest sl = new REPInterest();
            sl.ShowDialog();
        }
        private void crystalReportViewer1_ReportRefresh(object source, CrystalDecisions.Windows.Forms.ViewerEventArgs e)
        {
            this.Hide();
            InterestCalculation ala = new InterestCalculation();
            ala.ShowDialog();
        }
