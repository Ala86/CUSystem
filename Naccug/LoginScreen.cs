...
namespace WinTcare
{
    public partial class LoginScreen : Form
    {
       ...
       // public static string gcDatabaseName = "";
        ...
        {
            //    MessageBox.Show("This is the Databse Name " + cs);
            //    globalvar.gcServername = Convert.ToString(cuview.Rows[comboBox1.SelectedIndex]["server_name"]).Trim();
            //    MessageBox.Show("This is the Server Name " + globalvar.gcServername);
            //    globalvar.gcDatabaseName = Convert.ToString(cuview.Rows[comboBox1.SelectedIndex]["catalog_name"]).Trim();
            //    MessageBox.Show("This is the Databse Name " + globalvar.gcDatabaseName);
            //    globalvar.gcCompName = Convert.ToString(cuview.Rows[comboBox1.SelectedIndex]["com_name"]).Trim();
    ...
    public void BindDropDown()
        {
            ...
            XmlTextReader xmdatareader = new XmlTextReader("naccug1.xml");
            DataSet _objdataset = new DataSet();
            _objdataset.ReadXml(xmdatareader);
            comboBox1.DataSource = _objdataset.Tables[0];
            comboBox1.DisplayMember = "gcComname";
            comboBox1.ValueMember = "gnCompid";
            ...
            textBox5.DataBindings.Add("Text", comboBox1.SelectedItem, "gcComname");
            globalvar.gcComname = comboBox1.Text;
        }
        ...
        {
            //globalvar.gcServername = Convert.ToString(cuview.Rows[comboBox1.SelectedIndex]["gcServername"]).Trim();
            //MessageBox.Show("This is the Server Name " + globalvar.gcServername);
            //globalvar.gcDatabaseName = Convert.ToString(cuview.Rows[comboBox1.SelectedIndex]["gcDatabaseName"]).Trim();
            //MessageBox.Show("This is the Databse Name " + globalvar.gcDatabaseName);
            //globalvar .gcCompName = Convert.ToString(cuview.Rows[comboBox1.SelectedIndex]["gcComname"]).Trim();
            //globalvar.gnCompid = Convert.ToInt32(cuview.Rows[comboBox1.SelectedIndex]["gnCompid"]);
            //globalvar.gcspassword = Convert.ToString(cuview.Rows[comboBox1.SelectedIndex]["gcspassword"]).Trim();
            //globalvar.gcsusername = Convert.ToString(cuview.Rows[comboBox1.SelectedIndex]["gcsusername"]).Trim();
          ...
          cs = globalvar.cos;
           ...
           // MessageBox.Show("This is the connection string " + cs);
            ...
            {
            //   MessageBox.Show("This is the Connection " + cs);
                nConnHandle.Open();
                ...
                if (cUserDetails.HasRows == true)
                {
                    ...
                    if (duser == finUserID && dpass == finUserPASS)
                    {
                        ...
                        globalvar.gnCreditLimit = Convert.ToDecimal(cUserDetails["credlimitamt"]);
                        globalvar.gnDebitLimit  = Convert.ToDecimal(cUserDetails["debtlimitamt"]);
                        globalvar.gnLoanLimit   = Convert.ToDecimal(cUserDetails["loanlimitamt"]);
            ...
            using (SqlConnection ndConnHandle = new SqlConnection(cs))
            {
                ...
                if (grpRitesView.Rows.Count > 0)
                {
                    ...
                    for (int i = 0; i < grpRitesView.Rows.Count; i++)
                    {
                        ...
                        if (dfunid == 193) {globalvar.glMemberAddAcct = true; }
...
//                        if (dfunid == 102) { globalvar.glLoanWriteOff = true; }
                        ...
                        if (dfunid == 194) { globalvar.glLoanWriteOff = true; }
                        ...
                        if (dfunid == 103) { globalvar.glSysAdmin = true; }
                        //********************************************************************************************
                        //********************************************************************************************
                    }
                }
...
//        private void 
        private void cashAccount(string actnumb)
        {
            using (SqlConnection nConnHandle = new SqlConnection(cs))
            {
                string cashap = "select cacctname,nbookbal from glmast where cacctnumb ="+"'"+actnumb+"'";
                ...
                if (ds1 != null & ds1.Rows.Count>0)
                {
                 ...
                 //   MessageBox.Show("This is the Cash Balance" + globalvar.gcCashAccontBal);
                }
            }
        }
...
//                    MessageBox.Show("The company has been found step 3");
                    ...
                    globalvar.gnChargeoffloor = Convert.ToInt16(cCompDetails["chargeoffloor"]);
                    globalvar.gnWriteoffloor = Convert.ToInt16(cCompDetails["writeoffloor"]);
...
globalvar.glAcctRept = globalvar.glAttendance = globalvar.glSysAdmin = false; 
...
//                        MessageBox.Show("The transfer flag in loginscreen is " + globalvar.glTransfers);
        ...
        private void comboBox1_SelectedValueChanged(object sender, EventArgs e)
        {
            ...
            string j = comboBox1.SelectedValue.ToString();
            var y = comboBox1.Items[x];
