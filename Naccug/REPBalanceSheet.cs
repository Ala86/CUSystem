...
namespace WinTcare
{
    public partial class REPBalanceSheet : Form
    {
        string cs = globalvar.cos;

        ReportDocument rprt = new ReportDocument();
        public REPBalanceSheet()
        {
            InitializeComponent();
        }

        private void REPBalanceSheet_Load(object sender, EventArgs e)
        {
            BranchCombo();
            companyCombo();
        }
        ...
        public void GETDATA()
        {
            ...
            int ncomp = Convert.ToInt32(comboBox1.SelectedValue);
            // string tcCode = textBox1.Text.ToString().Trim();
            SqlConnection ndConnHandle1 = new SqlConnection(cs);
            rprt.Load(System.Windows.Forms.Application.StartupPath + "\\Reports\\CRPBalanceSheetReport.rpt");
            SqlCommand cmd = new SqlCommand("tsp_BalanceSheets", ndConnHandle1);
            cmd.CommandType = CommandType.StoredProcedure;
            cmd.Parameters.AddWithValue("@lnCompID", ncomp);
         ...
         //   cmd.Parameters.Add(new System.Data.SqlClient.SqlParameter("@FromDate", SqlDbType.Date)).Value = dateTimePicker1.Text;
            ...
            SPBalanceSheet ds = new SPBalanceSheet();
            sda.Fill(ds, "tsp_BalanceSheets");
