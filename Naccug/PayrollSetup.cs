...
namespace WinTcare
{
    public partial class PayrollSetup : Form
    {
...
//                DataTable ds2 = new DataTable();
                ...
                //************Getting member list                 
                string dsql3 = "select ccustcode,ltrim(rtrim(ccustfname))+' '+ltrim(rtrim(ccustlname)) as fullname from cusreg ";
                SqlDataAdapter da3 = new SqlDataAdapter(dsql3, ndConnHandle);
                DataTable ds3 = new DataTable();
                da3.Fill(ds3);
                if (ds3 != null)
                {
                    comboBox2.DataSource = ds3.DefaultView;
                    comboBox2.DisplayMember = "fullname";
                    comboBox2.ValueMember = "ccustcode";
                    comboBox2.SelectedIndex = -1;
                }
                else { MessageBox.Show("Could not find member list, inform IT Dept immediately"); }
            }
        }
        ...
        {
            if (TextBox1.Text!="" && Convert.ToInt16(comboBox1.SelectedValue)>0 ) //&& Convert.ToInt16(comboBox2.SelectedValue)>0)// && glPage01Page05)
            {
 ...
 //           string lcmemcode = comboBox2.SelectedValue.ToString();
        ...
        private void prodGrid_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                   using (var findform = new TclassLibrary.FindClient(cs, ncompid, cloc, 2, "Cusreg"))
                   {
